...
public class VeiculoBO {
	 ...
	 */
	public void alterar (String placaAntiga ,String placaAtual) throws SQLException{
		VeiculoDAO veiculoDAO = new VeiculoDAO();
		
		//RN A nova placa não pode sert igual a placa atual
		if (placaAtual.equals(placaAntiga)) {
			throw new SQLException("A nova placa não pode ser igual a placa atual"); //RN1
		}else if((placaAntiga.substring(3,7)) == (placaAtual.substring(3,7))){
				throw new SQLException("O número da placa não pode ser igual ao numero de placa antiga"); //RN2
			}else{
